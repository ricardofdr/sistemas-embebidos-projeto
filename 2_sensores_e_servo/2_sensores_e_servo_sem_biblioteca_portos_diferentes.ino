...
#define TRIGGER_PIN1 4 // PD4
#define ECHO_PIN1 5     // PD5

// Define pins for second ultrasonic sensor (using manual pulse timing)
#define TRIGGER_PIN2 2 // PD2
...
void setup()
{
  ...
  DDRD |= (1 << DDD4);  // Set PD4 (pin 4) as output for trigger
  DDRD &= ~(1 << DDD5); // Set PD5 (pin 5) as input for echo

  // Configure second sensor pins (PORTD)
  DDRD |= (1 << DDD2);  // Set PD2 (pin 2) as output for trigger
  ...
  PORTD &= ~(1 << PD4); // First sensor trigger LOW
  PORTD &= ~(1 << PD2); // Second sensor trigger LOW
...
long measureDistance(int triggerPin, int echoPin)
{
  ...
  if (triggerPin == TRIGGER_PIN1)
  {
    ...
    PORTD &= ~(1 << 4); // Clear PB2 (pin 10)
    delayMicroseconds(2);
    PORTD |= (1 << 4); // Set PB2 high
    delayMicroseconds(10);
    PORTD &= ~(1 << 4); // Set PB2 low
  }
  else if (triggerPin == TRIGGER_PIN2)
  {
    // Second sensor uses Port D
    PORTD &= ~(1 << 2); // Clear PD7 (pin 7)
    delayMicroseconds(2);
    PORTD |= (1 << 2); // Set PD7 high
    delayMicroseconds(10);
    PORTD &= ~(1 << 2); // Set PD7 low
  }
